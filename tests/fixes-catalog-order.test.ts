...
describe("corrections catalog ordering", () => {
  test("prioritizes current-year releases and manual additions, then older DRM, then the rest", () => {
    const ordered = sortCorrectionCatalog([
      ...
      { appid: "5", name: "Old regular", releaseDate: "2025-10-01", manualAddedAt: null, hasDrm: false, fixes: [{ ...fix, score: 500 }] },
    ]);

    expect(ordered.map((item) => item.appid)).toEqual(["3", "2", "4", "1", "5"]);
  });
});
